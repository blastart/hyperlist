...
export default class HyperList {
  ...
  static getMaxBrowserHeight () {
    ...
    // calculate.
    const style = 'position: absolute; height: 1px; opacity: 0'
    const html = `<div id="_height_test" style="${style}"><div style="height: 1e7px"></div></div>`
    document.body.insertAdjacentHTML('beforeEnd', html)
    const el = document.getElementById('_height_test')

    const maxElementHeight = el.firstChild.offsetHeight
  ...
  _computeScrollPadding () {
    ...
    return {bottom: padding(vals[0]), top: padding(vals[1])}
  }
